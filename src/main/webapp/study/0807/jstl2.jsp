...
<div class="container">
  ...
  <div>
  	...
  	5.문자열추출? <%=str.substring(0,5) %> / ${fn:substring(str, 0, 5)}<br/>
  	5_2.문자열추출? <%=str.substring(5) %> / ${fn:substring(str, 5, -1)}<br/>
  	6_1.문자(열)검색? <%=str.indexOf("o") %> / ${fn:indexOf(str, 'o')}<br/>
  	6_2.문자(열)검색? <%=str.lastIndexOf("o") %> / 없음<br/>
  	6_3.str변수의 값중에서 'o'문자가 들어가 있는 모든 위치?<br/>
  	...
  	6-4.str변수의 값중 'o'는 뒤에서부터 몇번째?(split/indexof/substring/substringAfter/substringBefore()으로 검색하기) 
  	...
  	7.문자열분리(split())? 'o'를 기준으로 분리<br/>
  		...
  		str변수안의 'o'문자의 개수는 ${fn:length(positions)-1}<br/>
