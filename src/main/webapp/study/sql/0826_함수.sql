...
select count(kor) as korCnt, sum(kor) as korSum, avg(kor) as korAvg from sungjuk;
 ...
 -- 반올림 : round()
 select round(124.456) as su, round(123.456) as res ;
 select round(124.456) as su, round(123.456,1) as res ;
 select round(124.456) as su, round(123.456,2) as res ;
 select round(124.456) as su, round(123.456,-1) as res ;
 
 -- 버림(절삭) : truncate()
 select round(124.456) as su, truncate(123.456, 0) as res ;
 select round(124.456) as su, truncate(123.456, 1) as res ;
 select round(124.456) as su, truncate(123.456, 2) as res ;
 select round(124.456) as su, truncate(123.456, -1) as res ;

 -- 무조건 올림: ceil(), 무조건 내림: floor()
select 123.456 as su,ceil(123.456) as res;
 select 123.456 as su,floor(123.456) as res;
 select 123.456 as su,floor(123.456,1x) as res;
 
 -- 나머지 : mod()
select mod(10,3) as na;
 
 -- 거듭제곱(지수승) : power()
  select power(2,5) 2의 5승;
 
 -- 숫자 리스트중에서 가장큰값(greatest), 가장 작근값(ㅣleast)
  select greatest(15,4,21,7,9) as max;
  select leatst(15,4,21,7,9) as min;
 
 -- 문자함수
 -- 문지열의 길이 : length() -바이트 단위
  select length('seoul');
  select length('서울');
...
-- 특정문자를 기준으로 이전까지만 출력
select substring_index('ab.cd.dfg.h.ijkl.mn', '.', 1);
select substring_index('ab.cd.dfg.h.ijkl.mn', '.', 2);
select substring_index('ab.cd.dfg.h.ijkl.mn', '.', 3);
...
-- 날짜 연산: date_add(), to_days(), datediff(), timediff(), timestampdiff() / interval : 오늘 날짜 기준으로 하루 넘김
