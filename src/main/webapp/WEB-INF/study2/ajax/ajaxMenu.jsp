  ...
  <pre>
  	...
  	* 프론트 프로그램에서의 비동기식	
  	...
  	☞ AJAX(ASynchronous Javascript And XML)
  		자바스크립트 라이브러리중에 하나이며, 브라우저객체인 XMLHttpRequest를 이용해서 전체페이지를 고치지 않아도
  		부분적인 페이지의 일부만을 처리할수 있도록 사용할 수 있게 해주는 기술이다.
  		즉, 자바스크립트를 이용하여 서버에 데이터를 요청할때 비동기식으로 통신하게 하는 방식이다.
  		과거에는 XML방식을 많이 사용하였으나, 프로그램 로직과 헤더의 변수를 사용하는 복잡한 방식이였기에,
  		지금은 JSON방식을 많이 사용할 수 있는 AJax방식을 많이 사용하고 있다.(JQuery에서 제공해주는 aJax라이브러리를 많이 사용한다.)
  		
  	<h5>AJAX에서의 전송방식(메소드) 종류</h5>
  	- GET : 데이터를 읽어주거나 주로 검색시에 사용
  	- POST : 새로운 리소스를 생성할 때 사용
  	- PUT : 새로 변경된 리소스를 생성/갱신(update)할때 사용
